...
// ---------------------------------------------------------------------------
// GET /api/books/isbn-set-lookup
//
// Given the ISBN of a boxed set / omnibus edition, guess which individual
// books it likely contains. THIS IS A HEURISTIC, NOT A LOOKUP OF FACT: no
// public API maps a boxset ISBN to its component ISBNs. We detect set-like
// language in the set's title (e.g. "Boxed Set", "Trilogy", "Books 1-3"),
// derive a probable series name, and search Open Library for other books by
// the same author matching that name. Results are best-effort matches for
// the user to review, not a verified table of contents.
// ---------------------------------------------------------------------------
...
router.get("/isbn-set-lookup", async (req, res) => {
  const isbn = req.query.isbn as string;
  if (!isbn) return res.status(400).json({ error: "isbn is required" });
  ...
  try {
    // Step 1: look up the boxset ISBN itself to get its title + author.
    const bookUrl = `https://openlibrary.org/api/books?bibkeys=ISBN:${cleanIsbn}&format=json&jscmd=data`;
    const bookResponse = await fetch(bookUrl, { signal: AbortSignal.timeout(8000) });
    const bookData = (await bookResponse.json()) as Record<string, unknown>;
    const key = `ISBN:${cleanIsbn}`;

    if (!bookData[key]) {
      return res.status(404).json({ error: "Book not found for this ISBN" });
    }

    const book = bookData[key] as Record<string, unknown>;
    const setTitle = (book.title as string) || "";
    const authors = Array.isArray(book.authors)
      ? (book.authors as { name?: string }[]).map((a) => a.name).filter(Boolean).join(", ")
      : "";
    const primaryAuthor = authors.split(",")[0]?.trim() ?? "";

    // Step 2: does the title actually look like a boxed set?
    const signal = detectSetSignal(setTitle);
    if (!signal.matched) {
      return res.json({
        isSet: false,
        setTitle,
        author: authors,
        message:
          "This ISBN doesn't look like a boxed set or omnibus edition — its title has no set-like language (e.g. 'Boxed Set', 'Trilogy', 'Books 1-3').",
        books: [],
      });
    }

    const seriesName = deriveSeriesName(setTitle);
    const estimatedCount = estimateVolumeCount(signal);

    if (!primaryAuthor || !seriesName) {
      return res.json({
        isSet: true,
        setTitle,
        ...
        matchedCount: 0,
        message:
          "This looks like a boxed set, but there wasn't enough author/title information to search for its individual volumes.",
        books: [],
      });
    }

    // Step 3: search for other books by this author matching the series name.
    const searchParams = new URLSearchParams({
      q: seriesName,
      author: primaryAuthor,
      fields: "title,author_name,first_publish_year,number_of_pages_median,subject,cover_i,isbn",
      sort: "old",
      limit: "40",
    });
    ...
    for (const doc of docs) {
      ...
      // Skip the boxset itself and any other omnibus/boxset-like results.
      if (looksLikeAnotherSet(docTitle)) continue;
      if (normalizeTitle(docTitle) === normalizeTitle(setTitle)) continue;

      // Loosely require the candidate title to belong to the same series —
      // all significant series words should appear in the candidate title.
      const lowerTitle = docTitle.toLowerCase();
      const matchesSeries =
        seriesWords.length > 0 && seriesWords.every((w) => lowerTitle.includes(w));
      ...
      const coverUrl = doc.cover_i
        ...
        : null;
      const representativeIsbn = doc.isbn?.[0] ?? `set-${cleanIsbn}-${candidates.length}`;

      candidates.push({
        isbn: representativeIsbn,
        status: "found",
        title: docTitle,
        author: doc.author_name?.join(", ") ?? primaryAuthor,
    ...
    // Sort by publish year (earliest first) so volume order is more likely
    // to match the series reading order; undated results go last.
    candidates.sort((a, b) => {
      if (a.publishYear == null && b.publishYear == null) return 0;
      if (a.publishYear == null) return 1;
      if (b.publishYear == null) return -1;
      return a.publishYear - b.publishYear;
    });

    const cap = estimatedCount ? Math.min(estimatedCount + 2, 20) : 12;
    const books = candidates.slice(0, cap);

    const confidence = computeConfidence(signal, estimatedCount, books.length);
    ...
    return res.json({
      ...
      message:
        books.length === 0
          ? "This looks like a boxed set, but no matching individual volumes could be found automatically. You may need to add them one by one."
          : null,
      books,
    });
  } catch (err) {
    console.error("[isbn-set-lookup]", err);
    return res.status(500).json({ error: "Failed to look up boxed set" });
  }
});
